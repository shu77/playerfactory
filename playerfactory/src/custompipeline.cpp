...
bool CustomPipeline::load(MEDIA_STREAMOPT_T *streamOpt, MEDIA_FORMAT_T mediaFormatType) {
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");
...
MEDIA_STATUS_T CustomPipeline::load(MEDIA_CUSTOM_SRC_TYPE_T srcType,
                                const gchar *pSrcPath,
                                const gchar *pWritePath,
                                guint64 startOffset,
                                MEDIA_CUSTOM_CONTENT_INFO_T *pstContentInfo)
{
    ...
    LMF_DBG_PRINT("========================================\n");
    LMF_DBG_PRINT("[%s:%d] ch:%d, SrcType: %d\n", __FUNCTION__, __LINE__, ch, srcType);
    ...
    LMF_DBG_PRINT(" size: %lld, esCh: %d, videoBuff [%d:%d], audioBuff [%d:%d]\n",  contentInfo.size,
                                                            contentInfo.esCh,
    ...
    {   
        ...
        switch(m_eSrcType)
        {
            case MEDIA_CUSTOM_SRC_TYPE_ES:
            {
                ...
                retVal = LMF_STATIC_ES_AddSinkElement(&gPipelineInfo[ch],srcType, contentInfo, pWritePath);
#endif 
            }break;
            case MEDIA_CUSTOM_SRC_TYPE_DOWNLOAD:
            {
#if 0 /* jhtark - webos refactory - temp [ 추가 필요 ] */
                retVal = LMF_STATIC_DN_AddSinkElement(&gPipelineInfo[ch],srcType, contentInfo, pWritePath);
#endif 
            }break;
            default:
            {
    ...
    */
    ...
    if (_LMF_STATIC_COMM_CreateThread(ch, srcType, contentInfo, pWritePath) != LMF_OK)
    {
...
guint8 CustomPipeline::_CheckContentType(void)
{
    ...
    LMF_DBG_PRINT("[%s:%d] vcodec= 0x%X, acodec= 0x%X\n",__FUNCTION__, __LINE__, m_stContentInfo.vcodec, m_stContentInfo.acodec);
    ...
    if(m_stContentInfo.vcodec != MEDIA_VIDEO_NONE)  // video
        m_eContentType = (CUSTOM_CONTENT_TYPE_T)(m_eContentType |CONTENT_TYPE_VIDEO);
...
MEDIA_STATUS_T CustomPipeline::_addSrcElement_Push(void)
{
    ...
    LMF_DBG_PRINT("[PUSH][BUFFER LEVEL][REQ] MAX : %u (MIN : %u bytes), PRE:%u\n",
    ...
    LMF_DBG_PRINT("[PUSH][BUFFER LEVEL][ADJ] MAX : %u (MIN : %u bytes / %u% ), PRE:%u\n",
        bufferMaxLevel, m_stContentInfo.videoDataInfo.bufferMinLevel, bufferMinPercent, preBufferLevel);
...
//  g_signal_connect(pSrcInfo->pSrcElement, "need-data", G_CALLBACK(_LMF_StartFeed), pSrcInfo);
...
#if 0 /* jhtark - webos refactory - temp [ 추후 진행  ] */ // changbok : need to check compile error
    ...
    if(retVal != LMF_OK)
    {
        LMF_ERR_PRINT("[%s:%d][PUSH] _LMF_STATIC_PKG_ConfigureMTKPipeline() return fail \n",__FUNCTION__, __LINE__);
        return retVal;
    }
#endif
#endif 
...
MEDIA_STATUS_T CustomPipeline::_addSrcElement(guint64 startOffset,const gchar *pSrcPath)
{
    MEDIA_SRC_T *pSrcInfo;
    MEDIA_STATUS_T  retVal = MEDIA_OK;
    ...
    switch(m_eSrcType)
    {
        ...
        case MEDIA_CUSTOM_SRC_TYPE_DOWNLOAD:
        {
            ...
            // protocol : HTTP only
            pSrcInfo->pSrcElement = gst_element_make_from_uri (GST_URI_SRC, pPath, "uri-source");
            if (!pSrcInfo->pSrcElement)
            {
            ...
            LMF_STATIC_COMM_SetPropertyOfProtocol((gchar *)pPath, pSrcInfo->pSrcElement);
            gst_bin_add(GST_BIN(gPipelineInfo[ch].pPipeline), pSrcInfo->pSrcElement);
#endif 
        }break;
        ...
        case MEDIA_CUSTOM_SRC_TYPE_SCREEN_SHARE:
        {
            ...
            retVal = LMF_STATIC_OTHER_AddUDPElement(&gPipelineInfo[ch],pcontentInfo);
            if(retVal != MEDIA_OK)
            {
        ...
        case MEDIA_CUSTOM_SRC_TYPE_RTP_MCAST:
        {
            ...
            retVal = LMF_STATIC_OTHER_AddFCCElement(&gPipelineInfo[ch], pcontentInfo,pPath);
            if(retVal != MEDIA_OK)
            {
...
void CustomPipeline::_callbackStopFeed(GstElement *element, MEDIA_SRC_T *app)
{
       ...
       (m_eNeedFeedData[app->srcIdx] != CUSTOM_BUFFER_LOCKED))
    {
        ...
        queuedSize = gst_app_src_get_queued_bytes(GST_APP_SRC(app->pSrcElement /*m_stSrcInfo[app->srcIdx].pSrcElement*/));
#endif 
    ...
    if ((m_stPrerollState.bUsePreroll == TRUE) &&
            (app->srcIdx == IDX_VIDEO) &&   // multi or video
            (m_stPrerollState.bIsPreroll == TRUE))
    {
...
gboolean CustomPipeline::_callbackSeekData(GstElement *element, guint64 offset, MEDIA_SRC_T *app)
{
...
void CustomPipeline::_callbackStartFeed(GstElement *element, guint32 size, MEDIA_SRC_T *app)
{
#if 0 /* jhtark - webos refactory - temp [ 아래과정 정리 필요  ] */
        MEDIA_CHANNEL_T ch = app->ch;
...
void CustomPipeline::_addNewPad(GstElement *element, GstPad *pad, gpointer data)
{
  char *pad_name;
  guint32 cnt;
...
/* changbok - webos refactory - temp [ 기존 ifdef 문 정리 필요 ] */
  ...
  if (g_strrstr(pad_name, "subtitle"))
  {
    ...
    if ((datalength>0) && (pPESDataBuffer!=NULL))
    {
      ...
      {
        LMF_SUBT_QueuingPESData((*datalength)+sizeof(guint), pPESDataBuffer);
      }
      else
      {
...
exit_addPad:
  ...
  if (pElementName != NULL) g_free(pElementName);
  if (pStrStructure != NULL)  g_free(pStrStructure);

  return;
}

void CustomPipeline::_addVideoPad(GstPad *pad)
{
  GstPad *sinkpad = NULL;
  ...
  {
    LMF_ERR_PRINT("%s.%d: _LMF_STATIC_COMM_AddVideoElement() return Error !!!\n", __FUNCTION__, __LINE__);
  ...
  if(retVal == MEDIA_ERROR)
  {
    LMF_ERR_PRINT("%s.%d: _LMF_STATIC_COMM_SetPropertyOnVideo() return Error !!!\n", __FUNCTION__, __LINE__);
//    goto exit_addVideoPad;
  }
...
#endif
*/    {
      if ((parent = GST_ELEMENT_CAST (gst_element_get_parent (pVideoQueueElement))))
      {
        ...
        * the parent */
        ...
        LMF_DBG_PRINT ("[video sink] syncing state (%s) to parent %s %s (%s, %s)\n",
...
//#if (PLATFORM_TYPE == LG_PLATFORM)
    ...
    g_object_set(G_OBJECT(pVideoDecoderElement), "vsink-async-handling", FALSE, NULL);
    g_object_set(G_OBJECT(pVideoDecoderElement), "vsink-sync-on-clock", FALSE, NULL);
    g_object_set(G_OBJECT(pVideoDecoderElement), "low-delay", TRUE, NULL);
//#endif
  }

  // set 3D Type
  LMF_DBG_PRINT("[%s:%d] 3D type : %d \n", __FUNCTION__, __LINE__, m_stContentInfo.videoDataInfo.interleaving_type);
  if(m_stContentInfo.videoDataInfo.interleaving_type != MEDIA_3D_NONE)
  {
    if ((g_object_class_find_property (G_OBJECT_GET_CLASS (pVideoDecoderElement), "interleaving_type")))
      g_object_set(G_OBJECT(pVideoDecoderElement), "interleaving_type", (m_stContentInfo.videoDataInfo.interleaving_type), NULL);
    else
...
void CustomPipeline::_addAudioPad(GstStructure *capsStr,GstPad *pad)
{

}

void CustomPipeline::_addTextPad(GstPad *pad)
{

}

void CustomPipeline::_findDemux(gchar **pDemuxName)
{
    int checkCnt = 0;
...
/* changbok - webos refactory - temp [ 기존 ifdef 문 정리 필요 ] */
    ...
    if(g_strrstr(pDemuxName, "mpegtsdemux")) // 20120119
    {
        ...
        g_object_set (G_OBJECT (gPipelineInfo[ch].pDemuxElement), "mem-add", &pPESDataBuffer, NULL);
    }
#endif //INCLUDE_ACTVILA
#endif
...
#endif
        {
            gst_element_link(gPipelineInfo[ch].pDrmElement, gPipelineInfo[ch].pDemuxElement);
        }
        else
        {
            gst_element_link(gPipelineInfo[ch].srcInfo[IDX_MULTI].pSrcElement, gPipelineInfo[ch].pDemuxElement);
        }
#endif
#endif
    }
...
MEDIA_STATUS_T CustomPipeline::FeedStream(guint8* pBuffer, guint32 bufferSize, guint64 pts, MEDIA_DATA_CHANNEL_T esData)
{
    ...
    /* changbok:Todo  */
#if 0 /* jhtark - webos refactory - temp [ static init 함수로 오는 매개변수 정리 필요  ] */
	if((LMF_STATIC_ES_GetEnableVideo() == TRUE) && (m_uNumOfSrc == 2))
	{
...
gint64 CustomPipeline::position() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}
gint CustomPipeline::volume() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}
gboolean CustomPipeline::isMuted() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}
gboolean CustomPipeline::isAudioAvailable() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}
gboolean CustomPipeline::isVideoAvailable() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}
gboolean CustomPipeline::isSeekable() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}
gfloat CustomPipeline::playbackRate() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");
...
GString CustomPipeline::errorString() const
{
    LOG_FUNCTION_SCOPE_NORMAL_D("CustomPipeline");

}

void CustomPipeline::playbinNotifySource(GObject *pObject, GParamSpec *pParam, gpointer u_data)
{
    CustomPipeline *genericPipeline = reinterpret_cast<CustomPipeline *>(u_data);
...
void CustomPipeline::handleVolumeChange(GObject *pObject, GParamSpec *pParam, gpointer u_data)
{


}
void CustomPipeline::handleMutedChange(GObject *pObject, GParamSpec *pParam, gpointer u_data)
{
