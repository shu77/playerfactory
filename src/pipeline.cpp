...
gboolean Pipeline::isSeekable(gpointer data)
{
	...
	if(self->m_isDLNA)
	{
		...
		{
			std::cout << "Is (DLNA - standard) Seekable = " << (((self->m_dlna_opval&0x11)==true)?"O":"X") 
               << "(opval byte based seek:" << ((self->m_dlna_opval&0x01)?"O":"X") 
               << "opval time based seek:" << ((self->m_dlna_opval&0x10)?"O":"X")
		...
		else if(MF_MAIN_GetLocale()==LMF_LOCALE_JAPAN && pPlayerHandle->bSeekableDuration)
		{
			LMF_DBG_PRINT("Is (DLNA JPN - nonstandard) Seekable = %s(opval byte based seek:%s,opval time based seek:%s,dur:%s)\n",
			(((pPlayerHandle->dlna_opval&0x11 || pPlayerHandle->bSeekableDuration)==true)?"O":"X"),
	...
	else // non DLNA case
	{
	    ...
	    std::cout << "Is Seekable = "
		<< (((self->m_seekable && bCheckSeekable && self->m_bSeekableIndex) == true)?"O":"X")
		<< "(state:"
		<< ((self->m_seekable == true)?"O":"X")
		<< ",dur+opval:"
		<< ((bCheckSeekable == true)?"O":"X")
		<< ",index:"
...
gint64 Pipeline::position() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");
...
gint Pipeline::volume() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");

}
gboolean Pipeline::isMuted() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");

}
gboolean Pipeline::isAudioAvailable() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");

}
gboolean Pipeline::isVideoAvailable() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");
...
void Pipeline::handleVolumeChange(GObject *pObject, GParamSpec *pParam, gpointer u_data){


}

void Pipeline::handleMutedChange(GObject *pObject, GParamSpec *pParam, gpointer u_data){


}

void Pipeline::setBufferController(BufferController::bsp_t busController) {
...
void Pipeline::collectTags (const GstTagList *tag_list, const gchar *tag, gpointer user_data)
{
...
void Pipeline::handleBusTag(gpointer data, GstMessage *pMessage)
{
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
...
void Pipeline::handleStateMsgPause(gpointer data, GstState oldState)
{
	...
	if (oldState == GST_STATE_READY)
	{
		...
		if (self->m_seekable == FALSE)
		{
			...
			*/
			if((self->m_source_format == MEDIA_FORMAT_FLV) &&
			   (self->isSeekable(self) == FALSE)) //only FLV case.
		...
		std::cout << "[BUS][CB - GST_STATE_PAUSED] oldstate - GST_STATE_READY 2" << endl;
	}
...
void Pipeline::handleBusStateMsg(gpointer data, GstMessage *pMessage)
{
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
	...
	if (self->m_playbinVersion == -1)
	{
		...
		std::cout << "[BUS] Get STATE Changed message..." << gst_element_state_get_name (oldState) << " -> " << gst_element_state_get_name (newState);
	...
	switch (newState)
	{
		...
		case GST_STATE_NULL:
		{
			...
			if (self->m_gstPipelineState != StoppedState)
			{
				...
				self->stateChanged(self->m_gstPipelineState);
				std::cout << "[BUS][CB GST_STATE_NULL]state changed - StoppedState " << endl;
			}
			else
			{
				std::cout << "[BUS][%s][%d][CB GST_STATE_NULL] state - StoppedState " << endl;
			}
		...
		case GST_STATE_READY:
		{
			...
			if (self->m_gstPipelineState != StoppedState)
			{
				self->m_gstPipelineState = StoppedState;
				self->stateChanged(self->m_gstPipelineState);
				std::cout << "[BUS][CB - GST_STATE_READY]state changed - StoppedState " << endl;
			}
			else
			{
...
void Pipeline::handleBusEOS(gpointer data)
{
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
	...
	{
		std::cout << "[BUS EOS][%s:%d] EOS callback came so early. >>> pending EOS." << endl;
...
gboolean Pipeline::handleBusElementMessage(gpointer data, GstMessage *pMessage){
	...
	std::cout << "[BUS]GST_MESSAGE_ERROR: " << (pErr->domain == GST_STREAM_ERROR? "GST_STREAM_ERROR":
			 	(pErr->domain == GST_CORE_ERROR? "GST_CORE_ERROR":
			  	(pErr->domain == GST_RESOURCE_ERROR? "GST_RESOURCE_ERROR":
			   	(pErr->domain == GST_LIBRARY_ERROR? "GST_LIBRARY_ERROR": "UNKNOWN")))) 
                  << " - " << pErr->code << " " << (GST_OBJECT_NAME(GST_MESSAGE_SRC(pMessage))) << " " << pErr->message << " " << pDebug << endl;

	/* error 중 무시되어야 하는 경우는 여기에 삽입 */
	if ((strncmp(GST_OBJECT_NAME(GST_MESSAGE_SRC(pMessage)), "queue2", strlen("queue2")) == 0) &&
...
gint64 Pipeline::getStreamLength(gpointer data)
{
	...
	if (self->m_duration == 0)
	{
		...
		gint64 len = -1;
#if (GST_VERSION_MAJOR >= 1)
		if (gst_element_query_duration(self->m_pipeHandle, queryFormat, &len) && (len != -1))
#else
		if (gst_element_query_duration(self->m_pipeHandle, &queryFormat, &len) && (len != -1))
#endif
		{
	...
	{
		std::cout << "[BUS][%s] try to get duration - element NULL or streamLength get already done. ";
	}

	return self->m_duration;
}

void Pipeline::handleBusDuration(gpointer data, GstMessage *pMessage)
{
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
	...
	{
		std::cout << "[BUS] update duration at GST_MESSAGE_DURATION [_gSessionDuration:" << (gint64)self->m_duration << endl;
	}
	else
	{
	...
	if ((duration >= 0) && (fmt == GST_FORMAT_BYTES))
	{
		...
		std::cout << "[BUS] filesize:" << duration << "G_GINT64_FORMAT:" << gst_format_get_name(fmt)<< endl;
		self->m_source_dataSize = (guint64)duration;
	}
}


void Pipeline::handleBusApplication(gpointer data, GstMessage *pMessage)
{
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
	...
	if (gst_structure_has_name(pStructure, "GstMessageLive"))
	{
		...
		bIsLive = g_value_get_boolean(gst_structure_get_value(pStructure, "LIVE"));
		...
		std::cout << "[BUS][GST_MESSAGE_APPLICATION] - is live of ASF:" << ((self->m_bNeedToRestart == true)?"OO":"XX") << endl;
	}
	else if (gst_structure_has_name(pStructure, "GstMessageSeekable"))
	{
		gboolean bIsSeekable = true;

		bIsSeekable = g_value_get_boolean(gst_structure_get_value(pStructure, "SEEKABLE"));
		self->m_bSeekableIndex = bIsSeekable;

		std::cout << "[BUS][GST_MESSAGE_APPLICATION] - seekable: " << ((bIsSeekable == true)?"OO":"XX") << endl;
	}
	else if (gst_structure_has_name(pStructure, "GstMessageAudio")) // demuxer가 invalid audio 로 판단
	{
		gboolean bValidAudio = true;

		bValidAudio = g_value_get_boolean(gst_structure_get_value(pStructure, "AUDIO"));

		std::cout << "[BUS][GST_MESSAGE_APPLICATION] - bValiadAudio:" << ((bValidAudio == true)?"OO":"XX") << endl;
	...
	else if (gst_structure_has_name(pStructure, "GstMessageVideo")) // demuxer가 invalid video 로 판단
	{
		...
		if(pValidVideo) // VIDEO message case. //(G_VALUE_TYPE (value) == G_TYPE_BOOLEAN)
		{
			...
			std::cout << "[BUS][GST_MESSAGE_APPLICATION] - bValidVideo:" << ((bValidVideo == true)?"OO":"XX") << endl;
...
gboolean Pipeline::gstBusCallbackHandle(GstBus *pBus, GstMessage *pMessage, gpointer data){
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
    //TODO: change PERI LOG print.
    std::cout << "Pipeline] bus callback msg - element:" << GST_OBJECT_NAME(GST_MESSAGE_SRC(pMessage)) << " | name:" << gst_message_type_get_name(GST_MESSAGE_TYPE(pMessage)) << endl;
...
void Pipeline::setSeekable(bool seekable){
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");
...
void Pipeline::setSeekable(gpointer data, bool seekable)
{
    Pipeline *self = reinterpret_cast<Pipeline*>(data);
	LOG_FUNCTION_SCOPE_NORMAL_D("Pipeline");
...
gboolean Pipeline::setPlaybackRate(gfloat rate)
{
