...
void CustomPlayer::playSpi(int rate) {
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
...
#if 0
void CustomPlayer::loadSpi(MEDIA_STREAMOPT_T *streamOpt, MEDIA_FORMAT_T mediaFormatType){
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
...
void CustomPlayer::loadSpi(MEDIA_CLIPOPT_T *clipOpt){
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
...
gint64 CustomPlayer::positionSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
	//return pipeline->position();
}

gint CustomPlayer::volumeSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
	//return pipeline->volume();
}

gboolean CustomPlayer::isMutedSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
	//return pipeline->isMuted();
}

gboolean CustomPlayer::isAudioAvailableSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
	//return pipeline->isAudioAvailable();
}

gboolean CustomPlayer::isVideoAvailableSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
	//return pipeline->isVideoAvailable();
}

gboolean CustomPlayer::isSeekableSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
	//return pipeline->isSeekable();
}

gfloat CustomPlayer::playbackRateSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
...
//Error error() const = 0;
GString CustomPlayer::errorStringSpi() const{
	LOG_FUNCTION_SCOPE_NORMAL_D("CustomPlayer");
	//Pipeline::bsp_t pipeline = getPipeline();
